...
exports.checkAccess = (req, res, next) => {
    ...
    if(req.session.isAdmin === false && routeCheck){
        if(routeCheck.response === 'redirect'){
            req.session.message = 'غیر مجاز لطفا به مدیر مراجعه کنید';
            req.session.messageType = 'danger';
            res.redirect('/admin');
            return;
        }
        if(routeCheck.response === 'json'){
            res.status(400).json({message: 'غیر مجاز لطفا به مدیر مراجعه کنید'});
        }
    }else{
        next();
    }
...
exports.getEmailTemplate = (result) => {
    ...
    $('#paymentMessage').text('با تشکر از خرید شما .');
...
exports.getData = (req, page, query) => {
    ...
    .catch((err) => {
        throw new Error('خطا در بازیابی محصولات');
    });
};
